...
export const queryAudit = query({
    ...
    handler: async (ctx, args) => {
        const { claimIds, playerEntityIds, itemKeys, from, to, page, pageSize } = args;
        ...
        for (const claimId of claimIds) {
            ...
            if (playerEntityIds &&
                ...
                q = ctx.db
                    .query("storageLogs")
                    .withIndex("by_claimId_and_playerEntityId_and_timestamp", (idx) => idx.eq("claimId", claimId).eq("playerEntityId", playerEntityIds[0]));
            }
            else {
                q = ctx.db
                    .query("storageLogs")
                    .withIndex("by_claimId_and_timestamp", (idx) => idx.eq("claimId", claimId));
            }
        ...
        if (itemKeys && itemKeys.length > 0) {
            ...
            if (parsed.length > 0) {
                filtered = filtered.filter((l) => parsed.some(([type, id]) => l.itemType === type && l.itemId === Number(id)));
            }
        }
        ...
        for (const l of filtered) {
            ...
            if (l.action === "deposit")
                entry.deposits += value;
            else
                entry.withdrawals += value;
            bucketMap.set(bucket, entry);
        }
        const sortedBuckets = [...bucketMap.entries()].sort((a, b) => a[0].localeCompare(b[0]));
        let cumulative = 0;
...
export const upsertFetchState = internalMutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            ...
            .withIndex("by_claimId_and_buildingEntityId", (idx) => idx
...
export const getStaleBuildings = internalQuery({
    ...
    handler: async (ctx, args) => {
        ...
        const all = await ctx.db
            .query("storageFetchState")
            .withIndex("by_claimId_and_buildingEntityId", (idx) => idx.eq("claimId", args.claimId))
            .collect();
